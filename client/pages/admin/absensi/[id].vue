<template>
  <div>
    <div class="page-header flex items-center gap-4">
      <NuxtLink to="/admin/absensi" class="btn-outline px-3 py-2">
    ...
    <div v-else-if="attendance" class="card">
      <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 pb-5 border-b border-gray-100">
        <div class="flex gap-4 text-sm">
          <span class="text-gray-500">Guru: <span class="font-semibold text-gray-900">{{ attendance.teacher?.full_name }}</span></span>
          <span class="text-gray-500">Total: <span class="font-semibold text-gray-900">{{ attendance.details?.length }} siswa</span></span>
        </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(detail, index) in attendance.details" :key="detail.id">
              ...
              <td class="font-mono text-xs text-gray-500">{{ detail.student?.nis }}</td>
              <td class="font-semibold text-gray-900">{{ detail.student?.full_name }}</td>
...
const countStatus = (status) => attendance.value?.details?.filter((d) => d.status === status).length || 0
const statusVariant = (status) => { const map = { HADIR: 'success', SAKIT: 'warning', IZIN: 'info', ALPHA: 'danger' }; return map[status] || 'gray' }
onMounted(async () => { try { const response = await attendanceService.getById(route.params.id); attendance.value = response.data } catch { toast.error('Gagal memuat detail absensi') } finally { loading.value = false } })
</script>
