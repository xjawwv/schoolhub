    ...
    <div v-else-if="attendance" class="card">
      <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 pb-5 border-b border-gray-100">
        <div class="text-sm text-gray-500">
          Total: <span class="font-semibold text-gray-900">{{ attendance.details?.length }} siswa</span>
        </div>
            ...
            <tr v-for="(detail, index) in attendance.details" :key="detail.id">
              ...
              <td>
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-semibold text-gray-900">{{ detail.student?.full_name }}</span>
                </div>
              </td>
      ...
      <div v-if="!attendance.details?.length" class="py-8">
        <AppEmpty icon="attendance" title="Belum ada siswa yang absen" description="Siswa belum melakukan check-in untuk sesi ini" />
      </div>
    </div>
  </div>
</template>
